#!/home/bolausson/watermeter/bin/python3
#
#
...
VISUALIZE = True
# Display single digits below merged single digits
...
# Save croped images and extracted features for review into "test_dir"
...
img_dir = "/home/bolausson/watermeter/images"
...
modelpath = "/home/bolausson/watermeter/model"
...
IFDB_URL = "https://openhab.local.olausson.de"
IFDB_PORT = 8086
IFDB_VERIFY_SSL = True
IFDB_ORG = "beo"
IFDB_TOKEN = ""
IFDB_BUCKET = "openhab/autogen"
...
big_box = {
                "warm": 
                    {
                        # Smaller number for X moves box more towards the right of the numbers
                        "upper_left_x": 725,
                        ...
                        # Threshold (the higher the less sensitive) 130
                        "thresh_min": 100,
                        "thresh_max": 500,
                ...
                "cold":
                    {
                        # Larger number for X moves box more towards the right of the numbers
                        "upper_left_x": 860,
                        ...
                        # Threshold (the higher the less sensitive) 120
                        "thresh_min": 105,
                        "thresh_max": 500,
...
def write_to_ifdb(ifdbc, m3, currenttime, mytz, certainty):
    ...
    |> range(start: 1) \
    |> filter(fn: (r) => r["_measurement"] == "{IFDB_MEASUREMENT}") \
    |> filter(fn: (r) => r["Location"] == "Flat") \
    |> filter(fn: (r) => r["Filtered"] == "Yes") \
    |> last() \
    |> yield(name: "last")'

    if_q_return = ifdbc_read.query(if_q)

    if_q_return_dict = {}
    try:
        for table in if_q_return:
            key = table.records[0].values["_field"]
            value = table.records[0].values["_value"]
            if_q_return_dict[key] = value
    except Exception as e:
        ...
        if DEBUG:
            ...
            cprint(if_q_return_dict, "green")
        try:
            w_old = if_q_return_dict["warm"]
        ...
        if DEBUG:
            print(f"INFLUXD - JSON body to be written to InfluxDB (DB: '{IFDB_BUCKET}', Measurement: '{IFDB_MEASUREMENT}') - FILTERED")
            pp.pprint(body_filtered)
            print(f"INFLUXD - JSON body to be written to InfluxDB (DB: '{IFDB_BUCKET}', Measurement: '{IFDB_MEASUREMENT}') - UNFILTERED")
            pp.pprint(body_unfiltered)
            print(f"INFLUXD - JSON body to be written to InfluxDB (DB: '{IFDB_BUCKET}', Measurement: '{IFDB_MEASUREMENT}') - CERTAINTY")
            pp.pprint(body_certainty)
            print(f"INFLUXD - JSON body to be written to InfluxDB (DB: '{IFDB_BUCKET}', Measurement: '{IFDB_MEASUREMENT}') - FILTERED + UNFILTERED + CERTAINTY")
            pp.pprint(body)
        
        if not DRYRUN:
            ifdbc_write.write(bucket=IFDB_BUCKET, org=IFDB_ORG, record=body)
...
def create_missing_feat(missing):
    canvas = np.ones((img_rows, img_cols, 1), np.float32)
    
    cv2font                = cv2.FONT_HERSHEY_SIMPLEX
    bottomLeftCornerOfText = (8,50)
    fontScale              = 1
    fontColor              = (0,0,0)
    lineType               = 3
    
    cv2.putText(canvas, missing, bottomLeftCornerOfText, cv2font, fontScale, fontColor, lineType)
...
def recognition(features, thresh, imgc, img_bb, woc):
    ...
    images_to_be_merged = {'reshaped': {}, 'Rec': {}}
    for pos in range(0, ndigits, 1):
        try:
            ...
            if VISUALIZE:
                images_to_be_merged['reshaped'][pos] = create_missing_feat("X")
                images_to_be_merged['Rec'][pos] = create_img("X")
                if VISUALIZE_SINGLE:
            ...
            predictions = model.predict(predictlist, verbose=predict_verbose)
            ...
            if VISUALIZE:
                image_from_best_guess = create_img(best)
                images_to_be_merged['reshaped'][pos] = feat
                images_to_be_merged['Rec'][pos] = image_from_best_guess
                
                if VISUALIZE_SINGLE:
                    cv2.imshow(visu_dict[woc]["Feature"][pos], feat)
                    cv2.imshow(visu_dict[woc]["Rec"][pos], image_from_best_guess)

    if VISUALIZE:
        im_feat_merge_list = [images_to_be_merged["reshaped"][pos] for pos in range(0, ndigits, 1)]
        im_rec_merge_list = [images_to_be_merged["Rec"][pos] for pos in range(0, ndigits, 1)]
        
        merged_found_features = np.concatenate(im_feat_merge_list, axis=1)
        merged_recognized_numbers = np.concatenate(im_rec_merge_list, axis=1)
 
        cv2.imshow(visu_dict[woc]["merged_features"], merged_found_features)
        cv2.imshow(visu_dict[woc]["merged_recognized"], merged_recognized_numbers)
...
def merge_images(images_to_merge):
    print("Merging images")
    #images_to_merge = [Image.open(x) for x in ['Test1.jpg', 'Test2.jpg', 'Test3.jpg']]
    widths, heights = zip(*(i.size for i in images_to_merge))
    ...
    new_im.save('merged_images.jpg')
    new_im_cv = cv2.cvtColor(np.array(new_im), cv2.COLOR_RGB2BGR)
...
if __name__ == "__main__":
    ...
    if VISUALIZE:
        ...
        for i, woc in enumerate(camsetup.keys()):
            space_between_warm_and_cold = 20 * i
            visu_dict[woc] = {}
            visu_dict[woc]["Feature"] = {}
            visu_dict[woc]["Rec"] = {}
            win_decoration_size = 100
            ...
            # Windows to display the merged extracted single digit features in
            merge_feat_x_pos = thresh_x_pos
            merge_feat_y_pos = thresh_y_pos + big_box[woc]["box_height"] + win_decoration_size
            winname_vis_merge_feat = f"{woc} - Merged features"
            visu_dict[woc]["merged_features"] = winname_vis_merge_feat
            cv2.namedWindow(winname_vis_merge_feat)
            cv2.resizeWindow(winname_vis_merge_feat, big_box[woc]["box_width"], big_box[woc]["box_height"])
            cv2.moveWindow(winname_vis_merge_feat,  merge_feat_x_pos, merge_feat_y_pos)

            # Windows to display the merged recognized values in
            merge_reco_x_pos = merge_feat_x_pos
            merge_reco_y_pos = merge_feat_y_pos + big_box[woc]["box_height"] + win_decoration_size
            winname_vis_merge_reco = f"{woc} - Merged recognized"
            visu_dict[woc]["merged_recognized"] = winname_vis_merge_reco
            cv2.namedWindow(winname_vis_merge_reco)
            cv2.resizeWindow(winname_vis_merge_reco, big_box[woc]["box_width"], big_box[woc]["box_height"])
            cv2.moveWindow(winname_vis_merge_reco,  merge_reco_x_pos, merge_reco_y_pos)
            ...
            if VISUALIZE_SINGLE:
                for j in range(0, ndigits, 1):
                    ...
                    feat_y_pos = merge_reco_y_pos + big_box[woc]["box_height"] + win_decoration_size
